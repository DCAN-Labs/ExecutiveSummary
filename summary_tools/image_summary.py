...
def get_list_of_tasks(proc_files):
    """
    Walks through the Results directory to find all the directories whose names
    contain 'task-'

    :param proc_files: path to the files generated by the pipeline.
    :return: list of tasks
    """
    tasklist=[]

    mni_path = os.path.join(proc_files, 'MNINonLinear/Results')
    if os.path.isdir(mni_path):
        print( '\nProcessed tasks will be found in path:\n\t%s' % mni_path)

        with os.scandir(mni_path) as entries:
        ...
        print('\nPath to processed task data does not exist; make sure prior processing has been done. Path:\n\t%s' % mni_path)
